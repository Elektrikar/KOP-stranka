        ...
        if ($error === '' && !empty($_FILES['image']['name'])) {
            ...
            if ($img['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($mime, $allowed, true)) {
                    ...
                    if (!empty($item['image'])) {
                        $oldFilename = basename($item['image']);
                        $oldPath = __DIR__ . '/img/category/' . $oldFilename;
...
$pageData = [
    'title' => ($type === 'category' ? 'Upraviť Kategóriu' : 'Upraviť Produkt') . ' | Admin',
    'metaDataDescription' => $type === 'category' ? 'Upraviť kategóriu' : 'Upraviť existujúci produkt',
    'customAssets' => [
...
<div class="admin-panel add-product-panel">
    <div class="admin-header">
        <h1><?= $type === 'category' ? 'Upraviť kategóriu' : 'Upraviť produkt' ?></h1>
        <a href="<?= $type === 'category' ? 'adminCategories.php' : 'adminProducts.php' ?>" class="back-link">Späť na zoznam <?= $type === 'category' ? 'kategórií' : 'produktov' ?></a>
    </div>
    ...
    <form method="post" enctype="multipart/form-data" class="product-form">
        ...
        <div class="form-group">
            ...
            <select name="category_id" required>
                ...
                <?php foreach ($categories as $c): ?>
                    <option value="<?= $c['id'] ?>"
                        <?= (($_POST['category_id'] ?? $item['category_id']) == $c['id']) ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" name="discount_price"
                   value="<?= htmlspecialchars($_POST['discount_price'] ?? $item['discount_price']) ?>">
        </div>

        <div class="form-group">
            <label>Množstvo na sklade</label>
            <input type="number" name="stock" min="0"
                   value="<?= (int)($_POST['stock'] ?? $item['stock']) ?>">
        </div>
        <?php endif; ?>

        <div class="form-group form-group-full">
            <label>Popis</label>
            <textarea name="description" rows="6"><?= htmlspecialchars($_POST['description'] ?? $item['description']) ?></textarea>
        </div>
        ...
        <div class="form-group form-group-full">
            ...
            <?php if ($item['image']): ?>
                <div style="margin: 10px 0;">
                    <img src="img/<?= $type === 'category' ? 'category' : 'productsmall' ?>/<?= htmlspecialchars($item['image']) ?>" 
                         alt="Aktuálny obrázok" 
                         style="max-width: 150px; max-height: 150px; border: 1px solid #ddd; padding: 5px; border-radius: 4px;">
                    <div style="margin-top: 5px; font-size: 0.9em; color: #666;">
                        Aktuálny obrázok: <?= htmlspecialchars($item['image']) ?>
                    </div>
                </div>
            <?php else: ?>
                <div style="margin: 10px 0; color: #666; font-style: italic;">
                    <?= $type === 'category' ? 'Kategória' : 'Produkt' ?> nemá priradený obrázok.
                </div>
            <?php endif; ?>
            ...
            <div style="font-size: 0.85em; color: #666; margin-top: 5px;">
                Povolené formáty: JPG, PNG. Obrázok sa zmenší na 500px<?= $type === 'product' ? ', malý na 60px' : '' ?>.
            </div>
        </div>
